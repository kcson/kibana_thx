...
import {Header, Form, Input, Tab, Icon, Table, Menu, Button, Select} from 'semantic-ui-react';
import {RIGHT_ALIGNMENT} from "ui_framework/services";
import moment from 'moment';

export default class AbnormalExternalIP extends React.Component {
  constructor(props) {
    super(props);
  ...
  renderCells(cells) {
    return cells.map((cell, index) => {
      ...
      if (index === 7) {
        content = <Button size="small" data-userId={cells[1]} data-type='external' onClick={(event, data) => this.handleButtonClick(data)}>상세 보기</Button>;
      } else {
        ...
        <Header style={{paddingTop: '18px'}} block={true}>
          <Form>
            <Form.Group inline>
              ...
              <Form.Field>
                <label style={{fontSize: '14px', width: '45px'}}>외부 IP</label>
                <Input style={{width: 'calc(100% - 65px)'}} placeholder='외부 IP'/>
              </Form.Field>
              <Form.Field>
